...
// Also mirror naming-seo to clean-code-seo-api for backward compatibility
geoTakeawaysData['naming-seo'] = geoTakeawaysData['clean-code-seo-api'];

// Write src/lib/geoTakeaways.ts
const tsContent = `/* ==========================================================================
...
export const geoLabelsByLocale: Record<Locale, { title: string; subtitle: string }> = ${JSON.stringify(geoLabelsByLocale, null, 2)};

export const geoTakeawaysData: Record<string, Record<Locale, string[]>> = ${JSON.stringify(geoTakeawaysData, null, 2)};
`;

fs.writeFileSync(path.join(projectRoot, 'src/lib/geoTakeaways.ts'), tsContent, 'utf-8');
console.log('✓ Updated: src/lib/geoTakeaways.ts with all 8 articles across 6 locales');

// Write scripts/geo-takeaways-data.js
const ssgContent = `/* ==========================================================================
   [GEO - Generative Engine Optimization Core Data - SSG ESM]
   - Covers all 8 Technical Guides
   - 100% Multilingual Coverage across 6 Locales: en, es, ja, de, fr, zh
   ========================================================================== */

export const geoLabelsByLocale = ${JSON.stringify(geoLabelsByLocale, null, 2)};

export const geoTakeawaysData = ${JSON.stringify(geoTakeawaysData, null, 2)};
`;

fs.writeFileSync(path.join(projectRoot, 'scripts/geo-takeaways-data.js'), ssgContent, 'utf-8');
console.log('✓ Updated: scripts/geo-takeaways-data.js with all 8 articles across 6 locales');
